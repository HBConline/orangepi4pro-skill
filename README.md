...
```
                    ...
                    │─────────────────────────────────────────────────│
                    │                                                 │
                    │  🤖 Claude Code  ─ 加载 ─  orangepi4pro skill  │
                    │  获取：引脚映射 · 驱动状态 · 命令 · NPC 管线    │
                    │                                                 │
                    │  ╔═══════════════════════════════════════════╗  │
                    │  ║  ①  MIPI 摄像头驱动                          │
                    │  ║  ─────────────────────────────────────      │
                    ...
                    │  ║  ─────────────────────────────────────      │
                    │  ║  📡 4-lane MIPI-CSI → /dev/video8 就绪      │
                    │  ╠═══════════════════════════════════════════╣  │
                    │  ║  ②  C++ NPU 推理引擎（板载编译）              │
                    │  ║  ─────────────────────────────────────      │
                    │  ║  生成：ai_guard.cpp + CMakeLists.txt          │
                    │  ║  ┌─────────────────────────────────────┐    │
                    │  ║  │ // OpenCV 采集帧 → NPU YOLOv5s 推理  │    │
                    │  ║  │ V4L2Camera cam("/dev/video8",640,480)│    │
                    │  ║  │ rknn_init(&ctx, "yolov5s.nb",...);   │    │
                    │  ║  │ if (person_detected) {               │    │
                    │  ║  │   digitalWrite(2, HIGH); // 蜂鸣器   │    │
                    │  ║  │   mosquitto_publish(mqtt, ..., json);│    │
                    │  ║  │ }                                   │    │
                    │  ║  └─────────────────────────────────────┘    │
                    │  ║  $ mkdir build && cd build                   │
                    │  ║  $ cmake .. && make -j4  # A733 八核并行    │
                    │  ╠═══════════════════════════════════════════╣  │
                    │  ║  ③  检测到 person → 三方输出                  │
                    │  ║                                              │
                    │  ║  ┌──────────────────┐  ┌──────────────┐     │
                    │  ║  │ GPIO Pin 7/ wPi 2 │  │ MQTT Push    │     │
                    │  ║  │ gpio write 2 1    │  │ topic:       │     │
                    │  ║  │ → 蜂鸣器 HIGH      │  │ home/camera/ │     │
                    │  ║  │ gpio write 2 0    │  │ alert        │     │
                    │  ║  │ → 蜂鸣器 LOW       │  │ {"person":1, │     │
                    │  ║  │                   │  │  "conf":0.87}│     │
                    │  ║  └──────────────────┘  └──────────────┘     │
                    │  ╠═══════════════════════════════════════════╣  │
                    ...
                    │  ║  生成 web_monitor.py：                        │
                    │  ║  /          → 实时 MJPEG 流 + 告警历史表      │
                    │  ║  /stream    → multipart JPEG 推流             │
                    │  ║  http://<board_ip>:8080  ← 手机/PC 浏览器    │
                    │  ╠═══════════════════════════════════════════╣  │
                    │  ║  ⑤  systemd 服务 ─ 开机自启，崩溃自恢复       │
                    │  ║  ─────────────────────────────────────      │
                    │  ║  $ cat /etc/systemd/system/ai-guard.service   │
                    │  ║  [Service]                                    │
                    │  ║  ExecStart=/home/orangepi/ai-guard/build/ai_guard│
                    ...
                    │  ║  AmbientCapabilities=CAP_SYS_RAWIO             │
                    │  ║  ─────────────────────────────────────      │
                    │  ║  $ sudo systemctl enable --now ai-guard        │
                    │  ║  $ sudo systemctl status ai-guard  # ● active │
                    │  ╚═══════════════════════════════════════════╝  │
                    │                                                 │
                    │    ✅ 10 分钟：自然语言 → 安防系统上线           │
                    │                                                 │
              ...
              │                             │                             │
    ┌─────────▼──────────┐     ┌────────────▼──────────┐     ┌───────────▼─────────┐
    │  MIPI Camera       │     │  40-Pin GPIO          │     │  Wi-Fi 6 + Ethernet │
    │  ────────────────  │     │  ────────────────     │     │  ──────────────────  │
    │  OV13850 / IMX219  │     │  Pin  7 → wPi  2 (LED)│     │  wlan0 / end0       │
    │  /dev/video8       │     │  Pin 11 → wPi  3      │     │                     │
    │  4-lane MIPI-CSI   │     │  Pin 13 → wPi  4      │     │  nmcli dev wifi     │
    │                    │     │  Pin 15 → wPi  5      │     │  connect SSID pwd   │
    └─────────┬──────────┘     └────────────┬──────────┘     └───────────┬─────────┘
              │                             │                             │
    ┌─────────▼──────────┐     ┌────────────▼──────────┐     ┌───────────▼─────────┐
    │  3 TOPS NPU        │     │  Buzzer 蜂鸣器        │     │  MQTT Broker        │
    │  ────────────────  │     │  ────────────────     │     │  ──────────────────  │
    │  YOLOv5s (person)  │     │  有源 3.3V            │     │  192.168.1.100:1883 │
    │  Pegasus → NBG     │     │  HIGH=响 LOW=停       │     │  Mosquitto / EMQX  │
    │                    │     │  🔊 🔊 🔊            │     │  topic: home/camera │
    └────────────────────┘     └───────────────────────┘     └───────────┬─────────┘
                                                                        │
                                                               ┌────────▼────────┐
